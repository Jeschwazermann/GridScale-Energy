...
export default function CustomerList() {
  ...
  /*
   * IMPORTANT:
   * Depend on the user's ID, not the entire user object.
   *
   * Supabase can provide a new user object for the same authenticated
   * user when a session is re-established. The ID hasn't changed,
   * so there is no reason to reload the customers.
   */
  ...
  useEffect(() => {
    ...
    const loadCustomers = async () => {
      ...
      /*
       * Component may have unmounted while Supabase was fetching.
       */
      if (cancelled) return;
